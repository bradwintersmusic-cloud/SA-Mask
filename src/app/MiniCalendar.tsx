...
export function MiniCalendar({ today }: { today: string }) {
  ...
  return <section className={styles.month} aria-label="Quick calendar navigation">
    ...
    <div className={styles.monthGrid}>
      ...
      {Array.from({length:days},(_,i)=>{const date=`${month}-${String(i+1).padStart(2,"0")}`;return <Link key={date} href={calendarLink(date)} prefetch={false} aria-current={date===today?"date":undefined} aria-label={`Open calendar for ${date}`}>{i+1}</Link>;})}
    </div>
